...
LLM call — too slow to block the end of every turn on. So the work is split:
  ...
  * This file (the hook) does the cheap, deterministic part — notice which
    ...
    article's CLAUDE.md hierarchy and is not tied to any repo's layout.
...
Three guards keep it well-behaved:
  * Recursion guard — the reflector spawns a headless `claude` whose own Stop
    hook lands right back here; HELPLINE_AILAYER_REFLECT_LOCK makes that no-op.
...
def _claude_md_areas(root: Path) -> set[str]:
    ...
    the repo root. These are the areas the CLAUDE.md hierarchy governs."""
    areas: set[str] = set()
...
def _touched_areas(root: Path) -> set[str]:
    """CLAUDE.md-governed areas with working-tree changes — the cheap signal."""
...
def _diff_fingerprint(root: Path, areas: set[str]) -> str:
    """Hash the diff of just the touched areas — so unrelated edits elsewhere
    don't churn the fingerprint and re-trigger reflections."""
