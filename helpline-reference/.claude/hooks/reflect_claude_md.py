...
Two safety properties:
  * Recursion guard — the headless `claude` it spawns would fire its own Stop
    ...
    launched with HELPLINE_AILAYER_REFLECT_LOCK=1; both this file and the hook
...
def _claude_md_areas(root: Path) -> set[str]:
    ...
    the repo root — the areas the CLAUDE.md hierarchy governs. Layout-agnostic,
    so the reflector works in any repo, not just one shaped like Helpline."""
    areas: set[str] = set()
...
def reflect() -> int:
    ...
    # the billing change in unrelated noise (and blow the truncation budget).
